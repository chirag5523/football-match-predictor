...
st.set_page_config(page_title="Arsenal FC Predictor", page_icon="🔴", layout="wide")
...
def get_sample_table():
    """Sample table for when Firestore is not available"""
    data = {
...
if page == "League Table":
    ...
    if db:
        try:
            ...
            df = pd.DataFrame(teams).sort_values('position') if teams else get_sample_table()
        except:
            df = get_sample_table()
    else:
        df = get_sample_table()
    
    fig = px.bar(df, x='Team', y='Points', title="Premier League Points", color='Points', color_continuous_scale='Reds')
